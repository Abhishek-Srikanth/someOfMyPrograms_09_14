...
void set_posib()
{
    // cout << "Will now set posibilities for each value" << endl;
...
void rfrequency()
{
    ...
    for(int i = 0; i < 9; ++i) // traversers from row 1-9
    {
        ...
        for(int k = 0; k < 9; ++k)
        {
            if(freq[k]==1)
            {
                n = k+1; // n holds value of number with ! frequency
                break;
            }
        }
        ...
        for(int j = 1; j < 8; j+=3)
        {
            ...
            if(n3!=0)       // if a frequency 1 value exists
            {
                ...
                for(int Ci = i-1; Ci<=i+1; ++Ci)
                {
                    for(int Cj = j-1; Cj<=j+1; ++Cj)
                    {
                        // every element in the sudoku
                        for(int k = 0; k < 9; ++k)
                        {
                            if(sudoku[Ci][Cj].posib[n3-1] != 0)
                            {
                                sudoku[Ci][Cj].val = n3;
...
/*****************************************************/
        ...
        cout << "And the complete solved sudoku is : \n\n";
...
/*****************************************************/
    ...
    cout << "Do you wish to save this sudoku solution(y/n) : ";
    cin >> ch;
    if(ch=='y' || ch == 'Y')
    {
